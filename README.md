...
**老师指正之后改用新技术**
- 异步函数不能直接放在setup里面 要封装才可以  
- 后端传给前端都是json字符串，如果只有一层axios可以自动转成js对象 ，如果多层就要做些配置才可以 应该前端负责处理数据，避免后端资源的浪费
- 按照规范来写代码 store service component
- 后端不应该通过CrossOrigin处理跨域，因为解耦开发，不应该暴露接口，所以前端实现
- 